...
func main() {
	...
	previewAction := func(tool string) string {
		action := fmt.Sprintf("change-preview(%s)", previewCmd(tool))
		if tool == "" {
...
var (
	fzfHelpOnce sync.Once
	fzfHelpText string
)

func fzfSupportsOption(opt string) bool {
	fzfHelpOnce.Do(func() {
		out, err := exec.Command("fzf", "--help").CombinedOutput()
		if err != nil && len(out) == 0 {
			return
		}
		fzfHelpText = string(out)
	})
	return strings.Contains(fzfHelpText, opt)
}
