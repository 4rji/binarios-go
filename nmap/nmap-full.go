...
func main() {
   ...
   tview.Styles.SecondaryTextColor = tcell.ColorLightGrey
    ...
    logPane.SetChangedFunc(func() { app.Draw() })
    ...
    go func() {
        ...
        streamCmdToTextView(logPane, "nmap", "-sS", "-sV", "-T4", "--top-ports", "1000",
            "-iL", outDir+"/hosts.txt", "-oN", outDir+"/ports.nmap")

        logLn("[3] SMB enumeration")
        streamCmdToTextView(logPane, "nmap", "--script", "smb-enum-shares,smb-os-discovery",
            "-p", "445", "-iL", outDir+"/hosts.txt", "-oN", outDir+"/smb.nmap")

        logLn("[4] SNMP info")
        streamCmdToTextView(logPane, "nmap", "-sU", "-p", "161", "--script", "snmp-info",
            "-iL", outDir+"/hosts.txt", "-oN", outDir+"/snmp.nmap")

        logLn("[5] Vuln scan")
        streamCmdToTextView(logPane, "nmap", "--script", "vuln",
            "-iL", outDir+"/hosts.txt", "-oN", outDir+"/vuln.nmap")
