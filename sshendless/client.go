...
func randStringBytes(n int64) []byte {
	message := []byte("Mr The_Asshat - Try harder.")
	// Repeat or trim the message to fit the requested length, then terminate with a newline.
	if n <= 0 {
		return append(message, '\n')
	}
	output := make([]byte, 0, n+1)
	for int64(len(output)) < n {
		remaining := n - int64(len(output))
		if remaining < int64(len(message)) {
			output = append(output, message[:remaining]...)
		} else {
